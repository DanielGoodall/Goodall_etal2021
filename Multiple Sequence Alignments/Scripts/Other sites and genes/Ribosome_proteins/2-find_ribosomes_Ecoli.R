...
ribo_fas <- read.fasta('ribosome_genes.fasta', seqtype = 'DNA', as.string = TRUE)
ribo_fas

ribo_fas2 <- readDNAStringSet('ribosome_genes.fasta')
ribo_fas2

# Write FASTQ format for BT2
fastq <- writeXStringSet(ribo_fas2, filepath = 'ribosome_genes.fastq', format = 'FASTQ')
#fastq
...
bowtie2(bt2Index ='Ecoli_genomes', 'ribo_Ecoli.sam',
        seq1 = 'ribosome_genes.fastq',
        seq2 = NULL,
...
scan <- scanBam(asBam('ribo_Ecoli.sam'))
bam <- scanBam('ribo_Ecoli.bam')
...
csv <- write.csv(bam_df, 'ribosome_Ecoli.csv')
csv
